...
require('nodetime').profile({
    accountKey: '6eb587440476a6fd46f783f2aebfbf7bff549f17', 
...
var latency_home = true;
var latency_index = true;
...
// ******* Start ******
Cache.cache_doublecheck(null, function(err, results) {
		console.log('***************end doublecheck cache******')});

setInterval(function() {Cache.cache(null, function(err, results) {
		console.log('***************end cache******')}) }, 100000000);
//use module.exports, then we could load directly
module.exports = function(app) {
	app.get('/', checkNotLogin);
  app.get('/', function(req, res) {
		if(latency_index) elapsed_time("start send_html()");
    Post.get(null, function(err, posts) {
      ...
      res.render('index', {
        ...
        img: posts,
			...
			if(latency_index) elapsed_time("end send_html2()");

    });
			if(latency_index) elapsed_time("end send_html()");
  });
	  ...
	  app.get('/bing/:req', function(req, res) {
		  ...
		  searchImg.searchImg(null, query, function(err, results_db) {
			  request.get({
				    ...
				    //res.send(results.d.results[0].MediaUrl);	//case-sensitive
  ...
  app.get('/home',  function(req, res) {
		if(latency_home) elapsed_time("start home()");
	    Post.get_home(req.session.user.name, function(err, posts) {
	      if (err) {
	        posts = [];
	      }
				console.log("test335"+req.session.user);
